...
const app = express()
                                            // 의존성 주입으로 발생하는 장점
const productService = new ProductService() // 1. new 한 번으로 모든 곳에서 재사용 가능 (싱글톤 패턴)
const cashService = new CashService()       // 2. 의존성 주입으로 몽땅 한꺼번에 변경 가능
const pointService = new PointService()     // 3. 의존성 주입으로 쿠폰 구매 방식이 포인트 결재로 안전하게 변경 가능
                                            // (부가설명)
                                            ...
                                            // ==> 이 역할을 대신 해주는 Nest.js 도구: IoC 컨테이너
                                            // (알아서 new 해서 넣어주는 예, 즉 DI 해줌)
